...
- REBIND a handler's procedure: address the EXISTING handler by its FQN `Catalog.X.Form.F.Handler.<Event>` (form-level) or `Catalog.X.Form.F.<ItemKind>.<ItemName>.Handler.<Event>` (item-level) and pass a `procedure` property with the new BSL procedure name. This only re-points an existing handler; to BIND a new event use create_metadata, to remove it delete_metadata. A handler FQN accepts no other property.
