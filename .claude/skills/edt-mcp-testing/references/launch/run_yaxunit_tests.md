...
- **No time-based result cache.** There is no TTL. A completed `junit.xml` is reused only to satisfy a re-call that fetches a previously reported **Pending** run, and only **once**; every later identical call starts a fresh launch (which deletes the old temp dir for that run-key first). So once you have fetched a result, re-running the same filter (e.g. after fixing the code) re-executes — it never serves a time-stale cache. **Caveat (ambiguous identical args):** if you were told **Pending** and never fetched it, the next identical call returns that completed report **once** (not a fresh run) before subsequent calls re-execute. To force a fresh run after an abandoned Pending, either change the filter (new run-key, no pending entry) or make one identical call to drain the pending result and then call again to re-execute. **`terminate_launch` does NOT clear this** — `PENDING_FETCH` lives inside `RunYaxunitTestsTool`, so terminating the Eclipse launch leaves the once-only result on disk to be served by the next identical call.
