...
- A `Catalog` or `ChartOfCharacteristicTypes` FQN additionally renders its "Predefined items" table (Name / Code / Description / Folder / Parent, with nesting shown via the Parent column), in BOTH the default and `full: true` views. Pass a single ITEM's own FQN (`Catalog.X.Predefined.ItemName`) instead of the owner FQN to render just that item's properties (Name / Code / Description / Folder / Parent, plus a nested-item count when it is a folder).
