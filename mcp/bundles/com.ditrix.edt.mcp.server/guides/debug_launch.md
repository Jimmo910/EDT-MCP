...
If the unlikely preflight→launch race still lets the 1003 modal appear, an armed auto-confirmer presses its default button ("Stop existing and start new" / "Завершить старую и запустить новую") on the EDT UI thread so the launch completes without a human — matched on the message body ('Debug session for project' / 'Сессия отладки для проекта'), never the generic 'Question' title.
