...
A predefined item is addressed by the SAME FQN create_metadata / modify_metadata use: `Catalog.X.Predefined.ItemName` or `ChartOfCharacteristicTypes.X.Predefined.ItemName`. The same two-phase preview/confirm applies. Deleting a FOLDER item cascades ALL its nested items (recursively) - the preview's `items` array lists the folder AND every cascaded descendant (name + kind), and the message reports the count, so check it before confirming. Like a form member, there is no reference cascade here (`blockingReferencesCount` is always 0 and `force` has no effect); the change persists to the OWNER's `.mdo` (the predefined content is a plain containment on the owner, not a separate resource).
