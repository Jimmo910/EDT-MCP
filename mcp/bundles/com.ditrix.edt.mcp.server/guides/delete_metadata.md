...
## Two-phase workflow
1. Preview (confirm omitted / false): returns the refactoring title, the refactoring items, and the affected references (referencingObject, reference feature, targetObject FQN) plus a count. When the node is still referenced by metadata the refactoring CANNOT auto-clean, the preview also marks `blocking=true` and lists those `blockingReferences` - a confirm=true delete would be refused unless force=true. Nothing is modified.
